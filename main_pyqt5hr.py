...
Aplikacija za usporedbu isplativosti BEV i ICE vozila.
"""
...
class BEVCalculatorPyQt5(QMainWindow):
    """PyQt5 aplikacija za BEV Calculator."""
    
    # --- Default Values ---
    VEHICLE_ENTRIES = [
        ("bev_name", "Naziv BEV vozila", "Tesla Model 3"),
        ("ice_name", "Naziv ICE vozila", "VW Golf 8"),
        ("bev_price", "Cijena BEV (€)", "40000"),
        ("ice_price", "Cijena ICE (€)", "25000"),
        ("subsidy", "Poticaji (€)", "9000"),
        ("bev_maintenance", "Održavanje BEV (€/godina)", "150"),
        ("ice_maintenance", "Održavanje ICE (€/godina)", "400"),
    ]
    USAGE_ENTRIES = [
        ("annual_km", "Godišnja kilometraža (km)", "15000"),
        ("bev_consumption", "Potrošnja BEV (kWh/100km)", "18"),
        ("ice_consumption", "Potrošnja ICE (l/100km)", "7"),
        ("fuel_price", "Cijena goriva (€/l)", "1.5"),
    ]
    CHARGING_TYPES = [
        ("home_charge_pct", "home_charge_price", "Kućno", "80", "0.15"),
        ("public_ac_pct", "public_ac_price", "Javno AC", "15", "0.40"),
        ("public_dc_pct", "public_dc_price", "Javno DC", "5", "0.65"),
    ]
    ...
    def __init__(self):
        ...
        self.setWindowTitle("Kalkulator isplativosti BEV")
    ...
    def _apply_styles(self):
        """Primijeni CSS stilove na aplikaciju."""
        c = self.colors
    ...
    def _create_profile_group(self):
        """Kreiraj grupu za upravljanje profilima."""
        group = QGroupBox("Učitaj profil")
        layout = QVBoxLayout()
        
        # Combo box
        self.profile_combo = QComboBox()
        self.profile_combo.addItems(self._get_profiles_list() or ["Nema spremljenih profila"])
        layout.addWidget(self.profile_combo)
        
        # Button
        load_button = QPushButton("Učitaj profil")
    ...
    def _load_theme_preference(self):
        """Učitaj preferiranu temu iz datoteke."""
        try:
    ...
    def _save_theme_preference(self):
        """Spremi preferencu teme u datoteku."""
        try:
    ...
    def _toggle_theme(self):
        """Promijeni temu mezi svetlom i tamnom."""
        ...
        # Ažuriraj dugme za temu
        self.theme_button.setText("☀️ Svijetla" if self.dark_mode else "🌙 Tamna")

    def _create_theme_group(self):
        """Kreiraj grupu za promjenu teme."""
        group = QGroupBox("Izgled")
        layout = QHBoxLayout()
        
        self.theme_button = QPushButton("🌙 Tamna" if not self.dark_mode else "☀️ Svijetla")
    ...
    def _create_form_group(self, title, entries):
        """Generička metoda za kreiranje form grupe."""
    ...
    def _create_vehicle_group(self):
        """Kreiraj grupu za podatke o vozilima."""
        return self._create_form_group("Podaci o vozilima", self.VEHICLE_ENTRIES)

    def _create_usage_group(self):
        """Kreiraj grupu za korištenje i potrošnju."""
        return self._create_form_group("Korištenje i potrošnja", self.USAGE_ENTRIES)

    def _create_charging_group(self):
        """Kreiraj grupu za postavke punjenja."""
        group = QGroupBox("Postavke punjenja BEV")
        layout = QGridLayout()
        
        # Headers
        layout.addWidget(QLabel("Tip punjenja"), 0, 0)
        layout.addWidget(QLabel("Udio (%)"), 0, 1)
        layout.addWidget(QLabel("Cijena (€/kWh)"), 0, 2)
    ...
    def _create_button_group(self):
        """Kreiraj grupu za akcijske gumbe."""
        group = QGroupBox("Akcije")
        layout = QHBoxLayout()
        
        # Calculate
        calc_button = QPushButton("Izračunaj")
        calc_button.setObjectName("calcButton")
        calc_button.clicked.connect(self._perform_calculation)
        layout.addWidget(calc_button)
        
        # Save
        save_button = QPushButton("Spremi profil")
        save_button.setObjectName("saveButton")
        save_button.clicked.connect(self._save_profile)
        layout.addWidget(save_button)
        
        # Delete
        delete_button = QPushButton("Obriši profil")
    ...
    def _create_output_panel(self):
        ...
        # Export button
        export_button = QPushButton("Izvezi kao PNG")
    ...
    def _create_results_group(self):
        ...
        group = QGroupBox("Rezultati")
        layout = QVBoxLayout()
        
        # Breakeven label
        self.breakeven_label = QLabel("Točka isplativosti: -")
        font = QFont()
        ...
        # Annual costs labels
        self.bev_annual_label = QLabel("Godišnji trošak BEV: -")
        self.ice_annual_label = QLabel("Godišnji trošak ICE: -")
    ...
    def _get_profiles_list(self):
        """Dohvati listu dostupnih profila."""
        try:
            profiles = [
    ...
    def _format_currency(value):
        """Formatiraj vrijednost kao hrvatsku valutu."""
    ...
    def _plot_cost_lines(self, ax, results):
        """Nacrtaj linije troškova."""
        bev_label = f"{self.bev_name} Ukupni Trošak"
        ice_label = f"{self.ice_name} Ukupni Trošak"
    ...
    def _plot_breakeven_point(self, ax, results):
        ...
        if break_even is not None and 0 < break_even < results['years'][-1]:
            ...
            ax.plot(break_even, break_even_cost, 'g*', markersize=20,
                    label=f'Točka isplativosti ({break_even:.2f} g.)', zorder=5)

    def _setup_graph_labels_formatting(self, ax):
        """Postavi labele i formatiranje grafikona."""
        ax.set_title(f"Usporedba troškova: {self.bev_name} vs. {self.ice_name}",
                     fontsize=14, fontweight='bold', color=self.colors['text_primary'], pad=15)
        ax.set_xlabel("godine", fontsize=11, color=self.colors['text_primary'], fontweight='bold')
    ...
    def _clear_graph(self, message="Izračunajte za prikaz rezultata"):
        """Očisti grafikon i prikaži poruku."""
        self.fig.clear()
        self.fig.patch.set_facecolor(self.colors['chart_bg'])
        ax = self.fig.add_subplot(111)
        self._setup_graph_background(ax)
    ...
    def _perform_calculation(self):
        """Izvrši kalkulaciju i ažuriraj prikaz."""
        try:
            ...
            if avg_elec_price is None:
                self.breakeven_label.setText("Greška: Provjerite postavke punjenja.")
                self._clear_graph("Greška u postavkama punjenja")
                return
            ...
            if numeric_params is None:
                self.breakeven_label.setText("Greška: Provjerite jesu li svi unosi ispravni.")
                self._clear_graph("Neispravni unosi")
                return
            ...
            if self.results["break_even_years"] is not None:
                if self.results["break_even_years"] == 0:
                    text = f"✅ {self.bev_name} je isplativiji od prvog dana!"
                else:
                    text = f"Točka isplativosti: {self.results['break_even_years']:.2f} godina"
                ...
                self.breakeven_label.setText(
                    f"❌ {self.bev_name} se ne isplati u zadanom periodu."
                )
                self.breakeven_label.setStyleSheet(f"color: {self.colors['error']}; font-weight: bold;")

            bev_cost_text = f"Godišnji trošak {self.bev_name}: {self._format_currency(self.results['bev_annual_cost'])} €"
            ice_cost_text = f"Godišnji trošak {self.ice_name}: {self._format_currency(self.results['ice_annual_cost'])} €"
            ...
            self.breakeven_label.setText(f"Greška: {str(e)}")
            self._clear_graph(f"Greška:\n{str(e)}")

    def _load_profile(self):
        """Učitaj profil iz JSON datoteke."""
        profile_name = self.profile_combo.currentText()
        if not profile_name or profile_name == "Nema spremljenih profila":
            return
        ...
        try:
            ...
            QMessageBox.information(self, "Uspjeh", f"Profil '{profile_name}' je učitan.")
        except Exception as e:
            QMessageBox.critical(self, "Greška", f"Greška pri učitavanju profila: {e}")

    def _save_profile(self):
        """Spremi trenutni unos kao profil."""
        profile_name, ok = self._get_input_dialog(
            "Spremi profil", "Unesite ime profila:"
        )
        ...
        try:
            ...
            # Osvježi listu profila
            profiles = self._get_profiles_list()
            self.profile_combo.clear()
            self.profile_combo.addItems(profiles or ["Nema spremljenih profila"])
            self.profile_combo.setCurrentText(profile_name)

            QMessageBox.information(self, "Uspjeh", f"Profil '{profile_name}' je spremljen.")
        except Exception as e:
            QMessageBox.critical(self, "Greška", f"Greška pri spremi profila: {e}")

    def _delete_profile(self):
        """Obriši profil."""
        profile_name = self.profile_combo.currentText()
        if not profile_name or profile_name == "Nema spremljenih profila":
            QMessageBox.warning(self, "Upozorenje", "Nije odabran nijedan profil za brisanje.")
            return

        reply = QMessageBox.question(
            self, "Potvrda brisanja",
            f"Jeste li sigurni da želite trajno obrisati profil '{profile_name}'?",
        ...
        try:
            if os.path.exists(file_path):
                ...
                self.profile_combo.clear()
                self.profile_combo.addItems(profiles or ["Nema spremljenih profila"])
                QMessageBox.information(self, "Uspjeh", f"Profil '{profile_name}' je obrisan.")
            else:
                QMessageBox.critical(self, "Greška", "Datoteka profila nije pronađena.")
        except Exception as e:
            QMessageBox.critical(self, "Greška", f"Greška pri brisanju profila: {e}")
    ...
    def _export_to_png(self):
        ...
        if self.results is None:
            QMessageBox.warning(
                self, "Upozorenje",
                "Nema podataka za izvoz. Prvo izračunajte rezultate."
            )
            return

        file_path, _ = QFileDialog.getSaveFileName(
            self,
            "Spremi grafikon kao PNG",
            "",
        ...
        try:
            ...
            QMessageBox.information(
                self, "Uspjeh",
                f"Grafikon je uspješno izvezen:\n{file_path}"
            )
        except Exception as e:
            QMessageBox.critical(self, "Greška", f"Greška pri izvozenju: {e}")
